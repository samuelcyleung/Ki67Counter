 ...
 * @author samuelc
 */
var ScoringFieldsAllocator = /** @class */ (function () {
    // do nothing
    function ScoringFieldsAllocator() {
        /**
         * return TRUE if they are all different
         * false if at least two of them are equal
         */
        this.allDifferent = function (a, b, c) {
            return !(a == b || a == c || b == c);
        };
    }
    /**
     ...
     * @param percent_high
     */
    ScoringFieldsAllocator.prototype.init = function (percent_negligible, percent_low, percent_medium, percent_high) {
        ...
        if (numFieldsRemaining == 0) {
            ...
            if (Math.max(percent_high == 0 ? 0 : Math.abs(percent_high - Math.max(percent_medium, percent_low, percent_negligible)), percent_medium == 0 ? 0 : Math.abs(percent_medium - Math.max(percent_high, percent_low, percent_negligible)), percent_low == 0 ? 0 : Math.abs(percent_low - Math.max(percent_high, percent_medium, percent_negligible)), percent_negligible == 0 ? 0 : Math.abs(percent_negligible - Math.max(percent_high, percent_medium, percent_low))) >= THRESHOLD) {
     ...
     */
    ScoringFieldsAllocator.prototype.getNumNegligible = function () {
        return this.numNegligible;
    };
    /**
     * return number of low fields allocated
     */
    ScoringFieldsAllocator.prototype.getNumLow = function () {
        return this.numLow;
    };
    /**
     * return number of medium fields allocated
     */
    ScoringFieldsAllocator.prototype.getNumMedium = function () {
        return this.numMedium;
    };
    /**
     * return number of high fields allocated
     */
    ScoringFieldsAllocator.prototype.getNumHigh = function () {
        return this.numHigh;
    };
    return ScoringFieldsAllocator;
}());
exports.ScoringFieldsAllocator = ScoringFieldsAllocator;
